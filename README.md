...
def write_csv(rows: Iterable[Sequence], path: str | Path, header: tuple[str, ...] | None = None) -> None:
    ...
    '''
    p = Path(path)
    rows=list(rows)#создаю список из полученных строк(которые представлены как список/кортеж)
...
def frecquencies_from_text(text: str) -> dict[str,int]:
    ...
    return Counter(tokens) #это специальный счётчик, который сам подсчитывает элементы (замена ручного подсчёта через словарь)
...
top=top_n(count_word) #возвращает список кортежей

write_csv(top, "data/report.csv", ["word", "count"])#записывает данные из top в виде csv, в указанный путь, с заголовком
